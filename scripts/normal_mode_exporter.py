...
def main():
    ...
    if args.input_format == "turbomole":
        ...
        raise RuntimeError('Unknown format "%s"' % args.input_format)
...
def initialize_io(input_path: str, output_path: str):
    ...
    if not os.path.exists(input_path):
        raise RuntimeError('Provided input path "%s" does not exist' % input_path)

    if os.path.exists(output_path):
        if not os.path.isdir(output_path):
            raise RuntimeError(
                'Provided output path "%s" is not a directory' % output_path
            )
    else:
...
def detect_input_format(input_path: str, input_format: str) -> str:
    ...
    raise RuntimeError(
        "Unable to auto-detect input format. Please specify it explicitly."
    )
...
def process_turbomole_input(input_path: str) -> Tuple[Molecule, List[NormalMode]]:
    ...
    while i < len(control_file_lines):
        ...
        if current_line.strip() == "$coord" or current_line.strip().startswith(
            ...
            if not "file=" in current_line:
                raise RuntimeError(
                    "Expected normal modes to be given in an external file"
                )
            ...
            if not "file=" in current_line:
                raise RuntimeError(
                    "Expected vibrational spectrum to be given in an external file"
                )
    ...
    if normal_mode_content == "":
        raise RuntimeError("Unable to find TurboMole's normal modes")
    if coordinate_content == "":
        raise RuntimeError("Unable to find equilibrium geometry")
    if vib_spectrum_content == "":
        raise RuntimeError("Unable to find TurboMole's vibspectrum info")
...
def process_molpro_input(input_path: str) -> Tuple[Molecule, List[NormalMode]]:
    ...
    if not os.path.isfile(input_path):
        raise RuntimeError(
            f"Molpro format expects an input _file_, but '{input_path}' isn't one"
        )
